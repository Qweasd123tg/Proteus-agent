...
`agent.py` — минимальный внешний `Workflow` v10 worker. Он не импортирует
...
cargo run -p proteus-module-protocol --bin proteus-component-conformance -- \
  ...
  --export '{"slot":"workflow","module_id":"python_agent_loop","contract_version":"v10","module_config":{}}' \
...
`{"status":"success","result":WorkflowOutput}`. Ошибка workflow использует
`{"status":"error","failure":WorkflowFailure}` и может вернуть проверяемое
обновление истории без ложного `WorkflowOutput`.

Этот Python-пример реализует success envelope; его ошибки передаются
через `ProtocolError` без terminal history update. Перед tools и успешным
завершением пример вызывает `host.history.checkpoint`: подтверждённая history
...
Каждый binding содержит обязательный `execution_call`: этот пример передаёт
...
Разрешённые callbacks определяются только парой `workflow/v10`:
