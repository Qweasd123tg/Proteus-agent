...
`agent.py` — минимальный внешний `Workflow` v6 worker. Он не импортирует
...
cargo run -p proteus-module-protocol --bin proteus-component-conformance -- \
  ...
  --export '{"slot":"workflow","module_id":"python_agent_loop","contract_version":"v6","module_config":{}}' \
...
Этот Python-пример реализует только success envelope; его ошибки передаются
через `ProtocolError` без history update. Возврат выполненной части истории
на ошибке реализован в reference `coding.codex_loop`.

Разрешённые callbacks определяются только парой `workflow/v6`:
