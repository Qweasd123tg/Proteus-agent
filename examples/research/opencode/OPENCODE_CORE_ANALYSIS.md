...
> Анализ по checkout `examples/source/opencode` от 2026-04-30. Клон
> обновляется, upstream рефакторится: часть file-ссылок ниже может указывать
> на удалённые/переименованные файлы (например, sql-store и projectors).
