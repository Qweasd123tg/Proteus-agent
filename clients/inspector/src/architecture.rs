...
use crate::types::*;
use crate::ui_utils::{compact_json, copy_to_clipboard, set_timeout};

const TOPOLOGY_MAP_ELEMENT_ID: &str = "topology-map-canvas";
const TOPOLOGY_MAP_VIEWPORT_ID: &str = "topology-map-viewport";
...
struct MapViewState {
    ...
    set_scale: WriteSignal<f64>,
    offset: ReadSignal<(f64, f64)>,
    set_offset: WriteSignal<(f64, f64)>,
    dragging: ReadSignal<bool>,
    set_dragging: WriteSignal<bool>,
    // (pointer_x, pointer_y, offset_x, offset_y) на момент mousedown
    drag_start: ReadSignal<(f64, f64, f64, f64)>,
    set_drag_start: WriteSignal<(f64, f64, f64, f64)>,
}
...
impl MapViewState {
    ...
    fn zoom(self, factor: f64, anchor: (f64, f64)) {
...
fn map_event_anchor(ev: &WheelEvent) -> (f64, f64) {
...
pub(crate) fn ArchitectureView() -> impl IntoView {
    ...
    let on_mermaid_rendered = wasm_bindgen::closure::Closure::<dyn FnMut(web_sys::Event)>::wrap(
        Box::new(move |_| {
...
struct SlotView {
    ...
    alternatives: Vec<TopologyModule>,
}
...
fn slot_views(snapshot: &TopologySnapshot) -> Vec<SlotView> {
...
fn pipeline_steps(snapshot: &TopologySnapshot, slots: &[SlotView]) -> Vec<PipelineStep> {
    ...
    for view in slots
        .iter()
        .filter(|view| matches!(view.category.as_str(), "orchestrator" | "pipeline" | "registry"))
    {
        ...
        steps.push(PipelineStep {
            ...
            source: view
                ...
                .unwrap_or_else(|| if missing { "missing".to_owned() } else { "config".to_owned() }),
            missing,
        });
    }
    steps
...
fn backend_views(snapshot: &TopologySnapshot, slots: &[SlotView]) -> Vec<BackendView> {
    slots
        .iter()
...
fn plugin_contributions(snapshot: &TopologySnapshot, plugin: &TopologyPlugin) -> Vec<ContributionChip> {
    ...
    for tool in &plugin.provides.tools {
        let (state, suffix) = snapshot
            ...
            .map(|candidate| match (candidate.enabled, candidate.registered) {
