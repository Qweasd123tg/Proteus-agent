...
use crate::api::{encode_query_component, get_json, post_json};
...
pub(crate) fn SettingsView() -> impl IntoView {
    ...
    let toggle_tool_cards = move |_| {
        ...
        spawn_local(async move {
            ...
            .await
            {
                Ok(StdioOutput::Response { ok: true, .. }) => set_status.set("сохранено".to_owned()),
                Ok(StdioOutput::Response { error, .. }) => {
...
fn context_map_path(session_dir: Option<&str>) -> String {
    match session_dir {
        Some(session_dir) => format!("/context?session_dir={}", encode_query_component(session_dir)),
...
fn context_compaction_tokens(report: &ContextCompactionReport) -> String {
    match (report.original_token_estimate, report.output_token_estimate) {
        (Some(before), Some(after)) => {
            format!("{} -> {}", format_token_count(before), format_token_count(after))
        }
