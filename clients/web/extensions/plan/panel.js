...
export function mount({ root, compact, services }) {
  ...
  compact.innerHTML = '<style>:host{display:flex;gap:2px;align-items:center;flex-wrap:wrap;max-width:26px}i{width:5px;height:5px;border-radius:50%;background:var(--border-strong,#555)}.completed{background:var(--text-main,#ddd)}.in_progress{background:var(--accent-blue,#79a9ff)}</style><span>☷</span>';
  let previous;
  return services['agent.session.read'].subscribe(snapshot => {
    ...
    list.replaceChildren(); compact.querySelectorAll('span,i').forEach(item => item.remove());
    ...
    if (!steps.length) { list.append(node('p', 'Агент ещё не составил план', 'muted')); compact.append(node('span', '☷')); }
