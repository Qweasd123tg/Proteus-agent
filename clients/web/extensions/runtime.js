...
export function createPanelRuntime({ manifest, root, services, storage, load = url => import(url), onError }) {
  ...
  const ready = (async () => {
    try {
      ...
      const result = await implementation.mount(Object.freeze({ root, services: Object.freeze(selected), storage, signal }));
