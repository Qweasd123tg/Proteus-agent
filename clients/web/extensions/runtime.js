...
export function createPanelRuntime({ manifest, root, compact, panel, panels, services, storage, load = url => import(url), onError }) {
  ...
  const ready = (async () => {
    try {
      ...
      const result = await implementation.mount(Object.freeze({ root, compact, panel, panels: scopedPanels, services: Object.freeze(selected), storage, signal }));
