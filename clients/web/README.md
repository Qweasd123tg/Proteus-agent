...
`/inspect/topology` и показывает каждый факт один раз: turn pipeline по
server-side `slot.category`/`slot.order`, backend slots с tool→backend
связями, slot cards с альтернативами, plugin contributions из `provides` с
вычисленным состоянием, единый tools список с фильтрами и warnings; runtime
Mermaid используется как copy/export action, не как основной renderer. Страница `/resume`
