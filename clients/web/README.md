...
`/user-input`, отменяет turn через `/cancel`, очищает history через `/clear` и
создаёт свежий чат через `/new-session`. Боковой список чатов читает только
sessions текущего workspace через `/sessions/current`, а страница `/resume`
