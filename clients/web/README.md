...
категориям. Страница `/architecture` (модуль `src/architecture.rs`) читает
`/inspect/topology` и показывает каждый факт один раз: интерактивную Map —
диагностический Mermaid-граф из `/inspect/topology.mmd`, отрендеренный в
браузере через mermaid.js (CDN, как MathJax; офлайн остаётся copy источника),
turn pipeline по server-side `slot.category`/`slot.order`, backend slots с
tool→backend связями, slot cards с альтернативами, plugin contributions из
`provides` с вычисленным состоянием, единый tools список с фильтрами и
warnings. Страница `/resume`
