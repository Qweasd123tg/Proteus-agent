...
сворачиваемый блок над ответом; reasoning начинается свёрнутым, чтобы длинное
thinking-содержимое не блокировало ленту. Streaming deltas буферизуются
короткими пачками на клиенте, финальный markdown кэшируется по версии
сообщения, а MathJax запускается только для новых финальных сообщений с
math-delimiters. Tool-карточки во время долгих команд показывают elapsed time.
Code-блоки в markdown имеют ярлык языка, copy и wrap-toggle (делегированный
