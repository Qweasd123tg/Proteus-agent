...
Иконки web, Inspector и поставляемых UI-панелей используют общий SVG-набор
`clients/common/assets/proteus-icons.svg`: сетка 20 × 20, линия 1.5,
скруглённые окончания и `currentColor`. Trunk копирует набор в `/assets/`;
компоненты подставляют имя символа и размер, геометрия остаётся в SVG.
Сторонние UI-расширения по-прежнему рисуют собственный `compact`.
