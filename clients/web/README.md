...
категориям. Страница `/architecture` читает `/inspect/topology`, сначала
показывает короткий runtime path, затем рисует diagnostic map связей
slots/plugins/modules/tools из `edges`, а runtime Mermaid использует как
copy/export action, не как основной renderer. Страница `/resume`
