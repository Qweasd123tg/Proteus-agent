...
`context-pack`, `codex-compactor`, `memory-pack`, `policy-pack`,
`renderer-pack` и `sqlite-memory` подхватываются автоматически.
...
for p in file-tools git-tools shell-tool rg-search direct-patch coding-workflow context-pack codex-compactor memory-pack policy-pack renderer-pack sqlite-memory; do
