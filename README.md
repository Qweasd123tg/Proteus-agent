...
Вызовы tool `shell` из TUI открываются в новом окне Ptyxis: там виден живой
вывод команды, окно закрывается после её завершения, а результат возвращается
агенту в текущий turn. При запуске ядра без `agent-tui` shell остаётся headless.
