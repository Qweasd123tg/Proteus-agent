...
Ptyxis для terminal-tools. Следующие вызовы создают вкладки только в этом
окне, не затрагивая окно с `agent-tui`: перед запуском печатается команда,
затем виден живой вывод, а после завершения вкладка остаётся открытой в
интерактивном shell; результат сразу
возвращается агенту в текущий turn. При запуске ядра без `agent-tui` shell
остаётся headless.
...
`/configs` запрашивает у app-server фактический summary текущего запуска:
путь конфигурации, workspace, profile/model, permission mode, выбранные modules,
enabled/registered tools и загруженные plugins.
