...
`~/.proteus/plugins/`. Клиенты (TUI, потенциально GUI/web) живут отдельными
...
**Клиенты:**
- `proteus-tui` — normal-scrollback terminal UI над `proteus server stdio`:
...
По умолчанию `proteus-tui` запускает агента в текущей директории терминала.
...
вызовы создают вкладки только в этом окне, не затрагивая окно с `proteus-tui`:
...
запущено без `proteus-tui`, shell остаётся headless.
...
```bash
./install.sh
# добавляет ~/.local/bin/proteus и ~/.local/bin/proteus-tui wrapper'ы
proteus init coding
proteus doctor
```
