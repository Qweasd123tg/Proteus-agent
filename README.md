...
Вызовы tool `shell` из TUI открываются в новой вкладке Ptyxis: там виден живой
вывод команды, а после завершения вкладка остаётся открытой в интерактивном
shell; результат сразу возвращается агенту в текущий turn. При запуске ядра
без `agent-tui` shell остаётся headless.
