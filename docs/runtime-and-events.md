...
visual-layer логикой. Текущая реализация перезапускает `agent server stdio` с
...
session directory. Команда также принимает путь прямо к `messages.jsonl`,
тогда TUI использует parent directory как session dir.
