...
`proteus-tui` по умолчанию скрывает `AssistantReasoningDelta`. Команда
...
(`.proteus/events.jsonl`) и затем в legacy workspace paths. Найденные
`TokenUsageUpdated` восстанавливают usage для этой session. Если event log
недоступен, `/context` показывает fallback-оценку по загруженной
`messages.jsonl` истории. На `/clear` локальные totals сбрасываются.
...
явным аргументом текущая реализация перезапускает `proteus server stdio` с
