...
Во внешнем TUI `/resume` должен быть app-client командой поверх app-server
protocol, а не visual-layer логикой. Runtime уже имеет
`resume_from_session_dir`, но текущий stdio transport пока не умеет
переключать session у уже запущенного app-server; для полноценного `/resume`
нужен отдельный request/response в `agent-contracts::app_protocol` или запуск
нового server process с выбранной session directory.
