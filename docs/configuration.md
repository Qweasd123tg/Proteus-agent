...
core-resident slot facade tools (`apply_patch`, `remember_fact`, `search`). `rg` приходит
...
решают, идти через SSE streaming path или через non-stream fallback. Если
провайдер/прокси не поддерживает SSE, явно укажите `stream = false`.
