...
`proteus-tui` по умолчанию использует текущую директорию терминала как workspace
и может запускаться без `--cwd`. Для коротких запусков есть launcher profiles:

```bash
proteus-tui --profile work
```
...
`--proteus-bin`, `--config`, `--cwd` перекрывают значения из profile.
...
`proteus-tui` поддерживает те же startup overrides и profile key
