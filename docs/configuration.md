...
`git_diff`, `apply_patch`, `write_file`, `shell`, `remember_fact`). `rg`
...
файловые (`read_file`, `write_file`, `list_dir`, `grep`), git helpers
(`git_status`, `git_diff`) и `shell` — живут в плагинах `file-tools`,
`git-tools` и `shell-tool`. `agent.coding.example.toml` уже включает полный
...
`native` использует встроенный Rust handler (`apply_patch`, `search`), но `ToolSpec` берёт из config. Handlers для `read_file`, `write_file`, `list_dir`, `shell` удалены — соответствующие tools теперь в плагинах (`file-tools`, `git-tools`, `shell-tool`), а не в runtime-catalog.
...
`AskUserQuestion`; app-server держит
turn открытым, TUI отрисует single-choice, `multiSelect`, options/custom input
универсально и вернёт ответы через `StdioRequest::UserInput`.
...
ответ UI-клиента на approval request. Значение `0` отключает timeout; это
дефолт для интерактивного TUI, чтобы approval prompt ждал пользователя, пока
он явно не ответит или не отменит turn. Если задано ненулевое значение и клиент
не ответил вовремя, request закрывается как `approved: false`, pending approval
удаляется, а turn продолжает работу с отказанным tool call. При shutdown
app-server также отклоняет все pending approvals.
