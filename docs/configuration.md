...
core-resident tools (`apply_patch`, `remember_fact`, `search`). Для полного
набора (`read_file`/`write_file`/`list_dir`/`grep`/`shell`) установите
плагины `file-tools` и `shell-tool` через `./install.sh`.
