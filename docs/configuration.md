...
`tools.enabled` включает tools по имени. Core регистрирует четыре host-side tool'а:
`apply_patch`, `search`, `remember_fact`, `request_user_input`. Остальные стандартные tools —
...
typed question round-trip через tool `request_user_input`; app-server держит
turn открытым, TUI отрисует вопросы/options/custom input универсально и вернёт
ответы через `StdioRequest::UserInput`.
