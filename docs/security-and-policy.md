...
HTTP boundary требует per-server local session token на всех non-trivial
endpoints (`/events`, `/send`, `/approval`, `/user-input`,
`/cancel`, `/mode`, `/model`, `/reasoning`, `/effort`, `/config`,
`/inspect/topology`, `/inspect/topology.map`, `/inspect/topology.mmd`,
