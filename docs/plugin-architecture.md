...
- `PluginRoot` и `PluginRegistry` v1 считаются стабильной ABI-поверхностью.
  Новые capabilities не добавляются в их layout/vtable. Для расширений
  используется optional exported symbol `agent_plugin_register_modules_v2`,
  который ядро пробует найти после v1-регистрации.
...
Добавочные plugin capabilities не должны требовать пересборки старых плагинов. Для этого v1 ABI (`PluginRoot` + `PluginRegistry`) не расширяется напрямую: новый плагин может экспортировать optional symbol `agent_plugin_register_modules_v2` и получить `PluginRegistryV2`, а старый плагин без этого symbol'а грузится как раньше.

Breaking changes в уже опубликованной v1 ABI всё ещё требуют пересборки соответствующих плагинов. Это не стоит прятать config-флагом: если layout/vtable реально несовместимы, "пропустить проверку" было бы undefined behavior. Config может управлять только политикой загрузки/отключения плагинов, а не безопасно чинить ABI mismatch.
...
Текущее состояние: `BuiltinModuleCatalog` в `crates/modular-agent/src/core/module_catalog.rs` хранит модули через унифицированный `register_module<T>` — все slot'ы лежат в одном `HashMap<(SlotId, String), ModuleEntry>` с open `SlotId`. `PluginRegistry` ABI v1 к плагинам покрывает 6 slots (`tool`, `renderer`, `policy`, `patch`, `search`, `memory`); optional `PluginRegistryV2` добавляет `context_provider` и declarative `memory_policy` через отдельный exported symbol. Loader регистрирует плагинные модули в те же `catalog` entries.
...
- NullSearch, NullPatchApplier, NoMemory, NoMemoryPolicy, AllowAllPolicy, SimpleContextBuilder, PlainRenderer.
...
- ✅ PluginRegistry v1 sabi_trait с `register_renderer`, `register_tool`, `register_approval_policy`, `register_patch_applier`, `register_search_backend`, `register_memory_store`.
- ✅ Optional PluginRegistryV2 entrypoint через symbol `agent_plugin_register_modules_v2` для добавочных capabilities без rebuild старых плагинов.
- ✅ Hello-world плагины: `hello-renderer`, `hello-tool`, `hello-policy-patch`
  (`hello-policy-patch` также экспортирует V2 demo для `context_provider` и
  declarative `memory_policy`).
- ✅ Реальные плагины: `file-tools` (register_tool), `rg-search` (register_search_backend), `direct-patch` (register_patch_applier), `sqlite-memory` (register_memory_store через rusqlite+FTS5 bundled).
- ✅ Builtin SQLite FTS5 memory store в ядре как reference impl (modules.memory = "sqlite").
...
- ✅ `context_provider` добавлен как вклад в `repo_aware` pipeline: плагин возвращает `Vec<ContextChunk>`, а budget/order/truncation остаются в ядре. Catalog отклоняет provider id, совпадающий с builtin provider id, чтобы избежать silent shadowing. Полный `ContextBuilder` как plugin пока не добавлен.
...
- По одному module: ✅ RgSearch → `rg-search`; ✅ DirectPatchApplier → `direct-patch`; дальше policy/etc.
