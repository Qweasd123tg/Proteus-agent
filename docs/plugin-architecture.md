...
- ✅ Реальные плагины: `file-tools` (register_tool), `sqlite-memory` (register_memory_store через rusqlite+FTS5 bundled).
- ✅ Builtin SQLite FTS5 memory store в ядре как reference impl (modules.memory = "sqlite").
- ✅ Политика дубликатов: для tool — builtin/configured побеждает плагин со skip+warning; для renderer / policy / patch / search / memory / memory_policy — bail при конфликте `(slot, id)`, loader переводит в stderr warning.
...
- ✅ `context_provider` добавлен как вклад в `repo_aware` pipeline: плагин возвращает `Vec<ContextChunk>`, а budget/order/truncation остаются в ядре. Полный `ContextBuilder` как plugin пока не добавлен.
