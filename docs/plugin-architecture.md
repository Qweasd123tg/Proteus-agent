...
Текущее состояние: `BuiltinModuleCatalog` в `crates/modular-agent/src/core/module_catalog.rs` хранит модули через унифицированный `register_module<T>` — все slot'ы лежат в одном `HashMap<(SlotId, String), ModuleEntry>` с open `SlotId`. `PluginRegistry` ABI к плагинам покрывает 6 slots (`tool`, `renderer`, `policy`, `patch`, `search`, `memory`); loader регистрирует плагинные модули в ту же `catalog` entries. `memory_policy` и `context_builder` пока только builtin — их sabi_trait требует FFI callback bridge.
...
- ✅ PluginRegistry sabi_trait с `register_renderer`, `register_tool`, `register_approval_policy`, `register_patch_applier`, `register_search_backend`, `register_memory_store`.
...
- ✅ Политика дубликатов: для tool — builtin побеждает плагин со skip+warning; для renderer / policy / patch / search / memory — bail при конфликте `(slot, id)`, loader переводит в stderr warning.
...
- 🔜 Добавление остальных slot'ов в PluginRegistry (memory_policy, context — требуют FFI-callback в ядро из плагина).
