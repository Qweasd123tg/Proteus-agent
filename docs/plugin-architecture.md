...
- ✅ PluginRegistry sabi_trait с `register_renderer` и `register_tool`.
- ✅ Hello-world плагины: `hello-renderer`, `hello-tool`.
- ✅ Реальный плагин-пример: `file-tools` (register_tool через PluginTool ABI).
- ✅ Политика дубликатов в `module_catalog`: builtin побеждает плагин, плагин логируется и скипается.
- ✅ Escape hatch `AGENT_PLUGINS_DISABLE=1` для тестов.
- 🔜 `plugin.toml` manifest рядом с .so (видимость плагина без загрузки).
- 🔜 Добавление остальных slot'ов в PluginRegistry (по мере sabi_trait-freeze).
