...
- ✅ Реальные плагины: `file-tools` (register_tool), `rg-search` (register_search_backend), `sqlite-memory` (register_memory_store через rusqlite+FTS5 bundled).
...
- По одному module: ✅ RgSearch → `rg-search`; дальше ReadFileTool/patch/policy/etc.
