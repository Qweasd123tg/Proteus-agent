...
| `ToolSpec.supports_parallel_tool_calls` | tool implementation / configured executor / MCP discovery | host batch и streamed workflow scheduling | обязательный boolean, независимый от safety; exposure сохраняет значение; default Rust constructor — `false` |
...
| `ModelStreamEvent::MessageCompleted` | model adapter | BoundModel, workflow stream, journal/replay | authoritative ordered prefix terminal messages; неизменные ids/content, идемпотентный duplicate, исходные part ids после terminal parsing; дельты не разрешают tool execution |
| `WorkflowModelStreamCursor` / `WorkflowModelStreamItem` | workflow host | любые workflow exports | `workflow/v13`, один cursor на invocation, completed items до terminal, общий cancellation/cleanup и tool safety path |
