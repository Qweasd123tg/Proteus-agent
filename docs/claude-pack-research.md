...
- `claude.explore`: read-only ориентация (`read_file`, `list_dir`, `grep`,
  Claude-like `Read`/`Glob`/`Grep`);
- `claude.edit`: узкие правки (`apply_patch`, `write_file`) плюс read/grep;
- `claude.verify`: проверка (`shell` первым), затем read/grep и, если нужно,
  дополнительный patch.

`claude_phased` намеренно не показывает backend tool `search` в Claude-like
профиле: у него аргумент `query`, а у `Grep` - `pattern`, и смешивание двух
поверхностей провоцирует лишние schema errors. Для file discovery pack держит
`Read`/`Glob`/`Grep` и workflow-owned лимит: после not-found/no-match разрешена
только одна сфокусированная recovery-попытка, затем агент должен ответить или
спросить точный путь.
