...
| Compactor | `HistoryCompactor` | `modules.compactor` | `none`, plugin-provided |
...
ToolExposure `claude_phased`. Он меняет порядок действий и видимый набор tools,
но не добавляет новых core slots и не обходит policy/orchestrator.

Чтобы активировать experimental pack, поставьте его через
`AGENT_INSTALL_EXPERIMENTAL=1 ./install.sh` (он копирует `.so` в
`~/.agent/plugins/<name>/`) и добавьте имена в `tools.enabled`.
...
этого model call. Durable session history и `messages.jsonl` через этот slot не
переписываются. Поэтому compactor подходит для summary/sliding-window/token
budget стратегий, но не заменяет `MemoryStore` и `MemoryPolicy`.

Плагинный workflow получает compactor только через host capability
`compact_history_json`. Это оставляет стиль workflow в плагине, а ядро даёт
общую точку для экономии контекста.
