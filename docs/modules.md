...
user turn и его ephemeral context остаются после summary, чтобы workflow
продолжал отвечать на актуальный запрос. В текущем ABI compactor не делает
собственный model request: это deterministic replacement-history слой поверх
существующего `CompactionInput`.
