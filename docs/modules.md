...
Модульность v0 означает выбор встроенной реализации через config. Все текущие реализации живут в `src/modules`, а строки выбора обрабатываются в `src/core/registry.rs`.
...
2. Разместить встроенную реализацию в `src/modules` или adapter в `src/adapters`.
