...
Ближайшая целевая реализация - `repo_aware`: project instructions, top-level
tree, manifest files, query term extraction, targeted filename/text search,
token budget и metadata со scores. Это прежде всего проверка plugin-ready
границы `ContextBuilder`: реализация должна быть заменяемой через
`modules.context` и не требовать правок `AgentRuntime`.
