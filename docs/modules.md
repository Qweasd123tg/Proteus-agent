...
идёт через optional `PluginRegistryV2`, поэтому старые плагины без memory
policy не требуют rebuild.
...
Плагины могут добавить provider в этот pipeline через `register_context_provider`
в optional `PluginRegistryV2`. Такой provider активируется явно: его id нужно включить в
`module_config.context.repo_aware.providers`. Полный `ContextBuilder` как dylib
пока не является plugin boundary: orchestration, byte budget и порядок chunks
остаются в `repo_aware` ядра.
