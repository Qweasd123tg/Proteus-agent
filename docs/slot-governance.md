...
| Subagents / cheaper model delegation | `Workflow`, `ModelAdapter`, tools/app-server | parallel spawn/wait/cancel contract позже | решение пересмотрено: slot `subagent` добавлен; основание — три правдоподобные реализации (opencode task-tool, codex collab, codex agent-jobs) и риск дублирования оркестрации во всех workflow-модулях; parallel-вариант контракта — planned вторая реализация |
