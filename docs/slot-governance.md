...
```text
codex-style profile
  workflow       = "codex.plan_execute"
  context        = "codex.repo_aware"
  search         = "codex.fuzzy_paths"
  policy         = "codex.exec_rules"
  patch          = "codex.verified_patch"
  tool_exposure  = "codex.deferred_tools"
  renderer       = "statusline"
```

Такой profile может имитировать стиль чужого agent-а, но каждая часть остаётся
заменяемой и проверяемой отдельно.
