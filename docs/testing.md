...
Experimental behavior pack `plugins/claude_pack` намеренно исключён из root
workspace: он нужен как baseline/исследование, но не должен удорожать обычный
test path. Его проверка запускается отдельно:
...
- session-level approval cache переиспользует exact calls с canonical JSON args
  и tool-wide approvals в том же cwd;
