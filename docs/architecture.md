...
4. Вызывается `Workflow::run(task, history, ctx)`. Production workflows живут в плагине `coding-workflow`; dylib-workflow общается с core через узкий `PluginWorkflowHost` (plugin.rs): `build_context_json`, `complete_model_json`, `execute_tool(s)_json`, `compact_history_json`, `select_tools_json`, `run_subagent_json` (+ `spawn`/`wait`/`cancel_subagent_json`), `emit_event_json`.
