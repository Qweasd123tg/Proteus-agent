...
значение. Панели меняют ширину одним шагом; содержимое и меню появляются
через короткие анимации opacity/transform, мобильный обзор выезжает справа.
Анимации учитывают системную настройку уменьшения движения. Размеры записываются
после завершения перетаскивания боковых панелей. Поле ввода автоматически растёт
по тексту до ограничения высоты, затем прокручивается; ручного изменения его
высоты нет. Модель и параметры запроса открываются слева, отправка — справа.
Во время хода доступны остановка и добавление следующего запроса в очередь.
Композер занимает место в потоке и не закрывает последние сообщения. На узких экранах обзор закрывается
по фону или Escape.
