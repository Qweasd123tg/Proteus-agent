...
cargo build --release --manifest-path "${project_dir}/Cargo.toml" --features context-pack/plugin-entrypoint
...
mkdir -p "${plugins_dir}"
for plugin in file-tools shell-tool rg-search direct-patch coding-workflow context-pack hello-renderer hello-tool hello-policy-patch sqlite-memory; do
