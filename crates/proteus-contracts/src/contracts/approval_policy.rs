...
/// Permission grants owned by one execution capability binding.
///
...
/// строки в grant store текущего execution binding. Policy видит их в
...
///
/// Root Turn получает новый execution context и новый grant store. Более узкий
/// child binding внутри того же logical execution также может получить пустой
/// store, чтобы authority не расширялась автоматически. Ядро учитывает
...
pub struct ExecutionPermissionGrants {
    granted: Mutex<BTreeSet<String>>,
}

impl ExecutionPermissionGrants {
...
pub struct PolicyContext {
    ...
    /// Execution-bound гранты, выданные через approval-gated tool results
    /// (см. [`ExecutionPermissionGrants`]).
...
mod tests {
    ...
    fn grants_deduplicate_and_snapshot_sorted() {
        let grants = ExecutionPermissionGrants::default();
