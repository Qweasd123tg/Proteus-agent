...
use crate::{
    ...
    core::{
        ...
        ModuleCatalogEntrySummary, ModuleSourceTopology, ModuleTopology, ModulesConfig,
        PendingApproval, PendingUserInput, RuntimeReloadReport, SessionStore, TopologyBuildInput,
...
fn config_builder_snapshot_from_topology(
...
fn validate_config_builder_modules(
    modules: &BTreeMap<String, String>,
...
fn set_module_slot(modules: &mut ModulesConfig, slot: &str, module_id: String) -> Result<()> {
    match slot {
...
fn config_builder_target_path(config_path: Option<&Path>) -> Option<PathBuf> {
...
fn validate_module_config_toml(
    module_config: &BTreeMap<String, BTreeMap<String, Value>>,
) -> Result<()> {
...
async fn persist_config_builder(path: &Path, config: &AppConfig) -> Result<()> {
