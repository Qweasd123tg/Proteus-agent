...
impl PluginPolicyAdapter {
    pub fn new(policy: PolicyObject, config: Value) -> Self {
        Self {
    ...
    pub fn from_shared(policy: Arc<PolicyObject>, config: Value) -> Self {
        Self {
            inner: policy,
            config,
        }
...
mod tests {
    ...
    fn invalid_json_from_plugin_is_denied() {
        let adapter = PluginPolicyAdapter::new(
            PluginApprovalPolicy_TO::from_value(BrokenJsonPolicy, TD_Opaque),
            serde_json::Value::Null,
        );
        let decision = adapter.evaluate(&make_call(), &make_ctx());
        match decision {
