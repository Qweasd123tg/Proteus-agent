//! Dylib plugin loader.
//!
//! Сканирует папку `~/.proteus/plugins/` на `*.so`/`*.dylib`/`*.dll`, загружает
...
pub struct PluginManifest {
    ...
    /// Требуемая версия proteus-contracts для информационных целей. Реальная
...
pub fn load_plugins_from_dir(
    ...
    if std::env::var_os("PROTEUS_PLUGINS_DISABLE").is_some() {
        return Vec::new();
    }
...
fn load_one_plugin_inner(
    ...
    // proteus-contracts, вот здесь это вылезет.
    lib_header
