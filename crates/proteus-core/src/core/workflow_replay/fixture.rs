...
fn parse_snapshot(opened: &TurnOpened, turn_id: TurnId) -> Result<SessionConfigSnapshot> {
    ...
    if snapshot.schema_version != SessionConfigSnapshot::SCHEMA_VERSION {
        bail!(
            "turn {turn_id} uses unsupported config snapshot schema_version {}; expected {}",
            snapshot.schema_version,
            SessionConfigSnapshot::SCHEMA_VERSION
        );
    }
    Ok(snapshot)
}
